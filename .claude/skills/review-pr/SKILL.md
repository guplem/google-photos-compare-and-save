...
Record: which areas the PR touches (`src/savedState/`, `src/instantSwap/`, `src/controlPanel/`, `src/settings/`, `options/`, `test/`, `adr/`, `.github/`, `manifest.json`), the total scope (additions/deletions, file count), the stated intent (commits + PR description), and any implementation proposal from the issue discussion.
