...
- **The two saved-state answers are not symmetrical.** Accept `unsaved` on the first clear reading and skip `minDwellMs`, because a half-drawn toolbar cannot invent a Save button. Make `saved` wait out `minDwellMs` and then hold for `confirmSavedMs`, because it is the absence of that button. Do not replace this with one symmetrical timeout, and do not make `unsaved` pay the dwell again: that fast path is what sets the speed of a whole scan.
...
- **The grid is the only way to see the album as a list, and it costs a scroll.** The page never holds more than about thirty thumbnails, so `albumGridReader` scrolls in steps and merges what appears. Stop the search at the first unchecked photo: reading the whole album to the bottom is pure waiting. Treat "every photo is known" as trustworthy only when the scroll actually reached the bottom, because running out of steps looks identical.
...
- **Google Photos hides the viewer chrome while the pointer stays still.** A scan never moves the real pointer, so the toolbar and the next-photo control both fade out. The probe then reads an empty toolbar and calls a readable photo unreadable. `keepPageAwake` dispatches a `mousemove` with changing coordinates; the coordinates must change, because a page that tracks the pointer ignores a move that lands twice on the same spot.
- **A fast loop outruns the album loading.** Google Photos loads a shared album in pages. Without `MIN_PHOTO_INTERVAL_MS` the scan steps through photos every few tens of milliseconds and arrives before the album has loaded that far, then reports a stall it caused itself. The floor covers **every** photo, not only the cached ones: charge it to one kind only and the faster kind ends up slower, which is exactly the bug that made a resumed scan slower than a first one. Keep the advance backoff (`ADVANCE_ATTEMPT_TIMEOUTS_MS`) generous at the later attempts too.
...
- **A stale toolbar may only cost a photo in the harmless direction.** On arrival the toolbar can still belong to the previous photo. A stale Save button badges a saved photo unsaved, and the user loses nothing because Google Photos deduplicates a re-saved file. A stale absence would badge an unsaved photo saved, and the user loses the photo. Every timing choice must keep that asymmetry.
