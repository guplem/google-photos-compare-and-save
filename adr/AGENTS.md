...
This directory holds the Architecture Decision Records for Google Photos Compare & Save.

## What is an ADR?

An ADR records one architectural decision or cross-cutting standard, with its context, the decision, and its consequences. It answers "how do we always do X here": where the saved-or-not signal comes from, how the code finds elements in a page it does not own, how the two content-script worlds talk, and what the extension stores where.
...
- A new setting, a new button name in the label lists, or a timing default. Those are code and belong in `AGENTS.md` rules at most.
